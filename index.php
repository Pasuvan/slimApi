...
$app->get('/getWsdl',function() use ($app){
	//$client = new SoapClient("http://localhost/spotify_api/soap/balanceservice.php?wsdl");
	$client = new SoapClient("http://www.webservicex.com/globalweather.asmx?WSDL");

	/* $apiID=2;$apiKey="0Hd33YAUmRu0";$userAction="10005";$userID="10042";$buyInAmount="100";
	$apiHashKey= md5($apiID."-".$apiKey."-".$userAction."-".$userID."-B2Bservice");

	$Balanceparams   = array("apiID"=>$apiID,"apiHashKey"=>$apiHashKey,"userAction"=>$userAction,"userID"=>$userID,"buyInAmount"=>$buyInAmount); 
	$BalanceResponse = $client->UserBalanceRequest($Balanceparams);
	*/
	
	$Balanceparams   = array("CountryName"=>'India','CityName'=>'new york');
	$BalanceResponse = $client->GetCitiesByCountry ($Balanceparams);

	echo "<pre>";print_r($BalanceResponse); 
	die;
});
